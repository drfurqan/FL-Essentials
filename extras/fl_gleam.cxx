//
// "$Id: fl_gleam.cxx 12779 2018-03-20 16:30:58Z AlbrechtS $"
//
...
/************************************************************************/
/* Modified for FL-Essentials - 09/24/2018                              */
/************************************************************************/
Fl_Color Fle_Color = fl_rgb_color(83, 160, 237);
static void fle_frame_rect(int x, int y, int w, int h, Fl_Color fg1)
{
	// outer border line:
	gleam_color(fg1);
	fl_xyline(x + 1, y, x + w - 2);   // top
...
static void fle_frame_rect_up(int x, int y, int w, int h, Fl_Color bc)
{
	fle_frame_rect(x, y, w, h, fl_color_average(fl_darker(bc), FL_WHITE, .65f));
}
static void fle_frame_rect_down(int x, int y, int w, int h, Fl_Color bc)
{
	fle_frame_rect(x, y, w, h, bc);
}
static void fle_up_box(int x, int y, int w, int h, Fl_Color c)
{
	shade_rect_top_bottom_up(x, y, w, h, c, .95f);
	fle_frame_rect_up(x, y, w, h, c);
}
static void fle_down_box(int x, int y, int w, int h, Fl_Color c)
{
	shade_rect_top_bottom_down(x, y, w, h, c, .85f);
	fle_frame_rect_down(x, y, w, h, Fle_Color);
}

extern void fl_internal_boxtype(Fl_Boxtype, Fl_Box_Draw_F*);

Fl_Boxtype fl_define_FL_GLEAM_UP_BOX() {
	fl_internal_boxtype(_FL_GLEAM_UP_BOX, fle_up_box);
	fl_internal_boxtype(_FL_GLEAM_DOWN_BOX, fle_down_box);
	fl_internal_boxtype(_FL_GLEAM_UP_FRAME, up_frame);
	fl_internal_boxtype(_FL_GLEAM_DOWN_FRAME, down_frame);
	fl_internal_boxtype(_FL_GLEAM_THIN_UP_BOX, fle_up_box);
	fl_internal_boxtype(_FL_GLEAM_THIN_DOWN_BOX, fle_down_box);
	fl_internal_boxtype(_FL_GLEAM_ROUND_UP_BOX, fle_up_box);
	fl_internal_boxtype(_FL_GLEAM_ROUND_DOWN_BOX, fle_down_box);
	return _FL_GLEAM_UP_BOX;
}


//
// End of "$Id: fl_gleam.cxx 12779 2018-03-20 16:30:58Z AlbrechtS $".
//
