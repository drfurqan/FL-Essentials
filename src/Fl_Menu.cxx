...
void Fl_Menu_Item::draw(int x, int y, int w, int h, const Fl_Menu_* m,
	...
	if (selected)
	{
		...
		//if (fl_contrast(r, color) != r) { // back compatibility boxtypes	// -
			if (0) { // back compatibility boxtypes							// +
		...
		if (selected == 2) { // menu title
			fl_draw_box(b, x, y, w, h, r);													// menu selection color
			x += 3;
			w -= 8;
		}
		else {
			fl_draw_box(b, x + 1, y - (LEADING - 2) / 2, w - 2, h + (LEADING - 2), r);		// item selection color
		}
	}
		...
		if (flags & FL_MENU_RADIO) 
		{
			...
			if (value()) {
				...
				if (Fl::is_scheme("gtk+")) {
					...
					tW--;
					fl_pie(x + td + 1 + gap, y + d + td - 1, tW + 3, tW + 3, 0.0, 360.0);
				...
				default:
					fl_pie(x + td + 2 + gap, y + d + td, tW, tW, 0.0, 360.0);
					break;

					// Small circles don't draw well on many systems...
				case 6:
					fl_rectf(x + td + 4 + gap, y + d + td, tW - 4, tW);
					fl_rectf(x + td + 3 + gap, y + d + td + 1, tW - 2, tW - 2);
					fl_rectf(x + td + 2 + gap, y + d + td + 2, tW, tW - 4);
					break;

				case 5:
				case 4:
				case 3:
					fl_rectf(x + td + 3 + gap, y + d + td, tW - 2, tW);
					fl_rectf(x + td + 2 + gap, y + d + td + 1, tW, tW - 2);
					break;

				case 2:
				case 1:
					fl_rectf(x + td + 2 + gap, y + d + td, tW, tW);
					break;
				}

				if (Fl::is_scheme("gtk+")) {
					fl_color(fl_color_average(FL_WHITE, FL_SELECTION_COLOR, 0.5));
					fl_arc(x + td + 2 + gap, y + d + td, tW + 1, tW + 1, 60.0, 180.0);
				}
			}
		}
		else 
	...
	: Fl_Menu_Window(X, Y, Wp, Hp, 0)
{
	...
	if (button) 
	{
		...
		/************************************************************************/
		box(button->box());												// -
		if (box() == FL_NO_BOX || box() == FL_FLAT_BOX) box(FL_FLAT_BOX);	// -
	...
	if (m) for (; m->text; m = m->next()) {
		...
		if (hh + LEADING > itemheight) itemheight = hh + LEADING; //+ 9;	// can change the menu items height/gap.
