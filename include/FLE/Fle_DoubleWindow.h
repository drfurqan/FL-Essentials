#pragma once
#ifndef Fle_DoubleWindow_h__
#define Fle_DoubleWindow_h__
...
class FL_ESSENTIALS_EXPORT Fle_DoubleWindow : public Fl_Double_Window
{
public:
	// Description:
	...
	Fle_DoubleWindow(int _x, int _y, int _w, int _h, const char* _title, int _icon_index = 101);
	// Description:
	...
	// _icon_index is the id of the icon that you want to display on the title bar.
	Fle_DoubleWindow(int _w, int _h, const char* _title, int _icon_index = 101);
	// Description:
	// Destructor that destroys timers (if any) and other data.
	virtual ~Fle_DoubleWindow();
	...
	void showNormal();
	// Description:
	// Function to show the window at the screen center if the given argument is true.
	virtual void show(bool _at_screen_center = false);
...
protected:
	...
	// int MyClass::keyPressEvent(int key)		// MyClass is the child class of Fle_DoubleWindow.
	// {
